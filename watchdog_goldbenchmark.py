"""
watchdog_cryptobenchmark.py -- Galahad for GoldBenchmark (Benchmark Desk).
Launches and supervises main_goldbenchmark.py: restarts it on unexpected exit,
...
TARGET = BASE_DIR / "main_goldbenchmark.py"

logging.basicConfig(level=logging.INFO,
                    format="%(asctime)s  %(levelname)-7s [Galahad] %(message)s",
                    datefmt="%Y-%m-%d %H:%M:%S")
logging.Formatter.converter = time.gmtime
log = logging.getLogger("GoldBenchmark.Watchdog")
